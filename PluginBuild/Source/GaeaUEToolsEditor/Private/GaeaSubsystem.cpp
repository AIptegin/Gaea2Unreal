			...
			if(bStatus)
			{
			   ...
			   GaeaDefinition.ScaleX, GaeaDefinition.ScaleY, GaeaDefinition.Height, GaeaDefinition.Resolution);
				...
				if(EditLayers)
				{
					...
					FGuid ID = BaseLayer ? BaseLayer->GetGuid() : FGuid();
#else
										...
										for (int32 i = 1; i < InfoObjects.Num(); i++)  // Start from 1 to skip first layer
										{
											...
											if (WeightmapIndex < GaeaComponent->WeightmapFilepaths.Num())
											{
												...
												if (ProxyWeightData.Num() == CompSizeX * CompSizeY)
												{
													...
													{
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
														ULandscapeEditLayerBase* BaseLayer = Landscape ? Landscape->GetEditLayer(0) : nullptr;
														FGuid ID = BaseLayer ? BaseLayer->GetGuid() : FGuid();
#else
														FLandscapeLayer* BaseLayer = Landscape ? Landscape->GetLayer(0) : nullptr;
														FGuid ID = BaseLayer ? BaseLayer->Guid : FGuid();
#endif
														LandscapeEdit.SetEditLayer(ID);
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
	ULandscapeEditLayerBase* Layer = Landscape ? Landscape->GetEditLayer(Index) : nullptr;
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
	bool bHasLandscapeLayersContent = true;
#else
	...
	for (ULandscapeComponent* NewComponent : NewComponents)
	{
		if (bHasLandscapeLayersContent)
		{
			...
			for (ULandscapeEditLayerBase* Layer : Landscape->GetEditLayers())
			{
				...
				NewComponent->AddDefaultLayerData(Layer->GetGuid(), ComponentsUsingHeightmap, CreatedHeightmapTextures);
			}
#else
...
void UGaeaSubsystem::CreateLandscapeActor(UImporterPanelSettings* Settings)
{
	...
	if(ImportWeightmaps)
	{
		...
		for(int i = 0; i < Settings->LandscapeMaterialLayerNames.Num(); i++)
		{
			...
			{
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
				LayerInfoObj->SetLayerName(Settings->LandscapeMaterialLayerNames[i], false);
#else
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
	// In UE5.8+ all landscapes use the edit layer system
#else
	...
	for(int32 i = 0; i < ImportLandscapeLayersList.Num(); i++)
	{
		...
		{
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
			Landscape->AddTargetLayer(ImportLandscapeLayersList[i].LayerName, FLandscapeTargetLayerSettings(ImportLandscapeLayersList[i].LayerInfo, ImportLandscapeLayersList[i].SourceFilePath));
#else
